...
const formatDownloadData = (apiData) =>
  apiData.map((ele) => {
    ...
    return {
      ...
      Price: ele?.lead?.exactValue ?? ele?.lead?.actualPrice ?? ele?.lead?.price,
      'Negotiation Amt': ele?.lead?.negotiatedAmount ?? 0,
      'Coupon Amt': ele?.lead?.couponDiscount ?? 0,
      'Bonus Amt': ele?.lead?.slabApplied ? ele?.lead?.slabBonusAmount : 0,
      'Final Price Offered to Customer': Number(ele?.lead?.price || 0) + Number(ele?.lead?.negotiatedAmount || 0) + Number(ele?.lead?.couponDiscount || 0),
