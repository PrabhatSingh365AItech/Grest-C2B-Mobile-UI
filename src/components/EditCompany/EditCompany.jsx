...
const SlabRow = ({ slab, index, onChange, onRemove }) => (
  <div className='flex flex-col gap-1'>
    <div className='flex gap-2 items-center'>
      <input type='number' className='border border-gray-300 px-2 py-1.5 rounded-lg outline-none w-[100px] text-sm focus:border-[var(--primary-color)] transition' value={slab.minValue} onChange={(e) => onChange(index, 'minValue', e.target.value)} placeholder='Min' />
      <span className='text-gray-400'>-</span>
      <input type='number' className='border border-gray-300 px-2 py-1.5 rounded-lg outline-none w-[100px] text-sm focus:border-[var(--primary-color)] transition' value={slab.maxValue} onChange={(e) => onChange(index, 'maxValue', e.target.value)} placeholder='Max' />
      <input type='number' className='border border-gray-300 px-2 py-1.5 rounded-lg outline-none w-[100px] text-sm focus:border-[var(--primary-color)] transition' value={slab.bonusAmount} onChange={(e) => onChange(index, 'bonusAmount', e.target.value)} placeholder='Bonus' />
...
const EditCompany = ({ companyData, setEditBoxOpen, setEditSuccess }) => {
  ...
  useEffect(() => {
    const fetchPricingConfig = async () => {
      ...
      try {
        const res = await axios.get(`${import.meta.env.VITE_REACT_APP_ENDPOINT}/api/dynamic-pricing/config/${companyData._id}?_t=${Date.now()}`, { headers: { Authorization: token } })
  ...
  const handleChange = (e) => {
    ...
    if (name === 'showPrice' || name === 'maskInfo') newValue = value === 'true'
      ...
      <form onSubmit={handleSubmit} className='px-6 pb-6 flex flex-col gap-4'>
        ...
        <div className='border border-gray-200 rounded-lg p-4'>
          ...
          <div className='flex flex-col gap-3 mb-4'>
            ...
            <select className='border border-gray-300 px-3 py-2 rounded-lg outline-none bg-white focus:border-[var(--primary-color)] transition text-sm w-[120px]' value={dynamicPricingEnabled.toString()} onChange={(e) => handleDynamicPricingToggle(e.target.value)}>
          ...
          <div className='flex flex-col gap-4 border-t border-gray-100 pt-4'>
            ...
            <button type='button' onClick={addSlab} className='self-start bg-gray-50 border-2 border-dashed border-gray-300 px-4 py-1.5 rounded-lg text-sm text-gray-600 hover:bg-gray-100 hover:border-gray-400 transition'>+ Add Slab</button>
            <div className='grid grid-row-2 gap-4'>
              <div className='flex flex-col gap-1.5'>
                ...
                <input type='date' className='border border-gray-300 px-3 py-2 rounded-lg outline-none focus:border-[var(--primary-color)] transition text-sm' min={new Date().toISOString().split('T')[0]} value={effectiveFrom} onChange={(e) => setEffectiveFrom(e.target.value)} />
              </div>
              <div className='flex flex-col gap-1.5'>
                <span className='font-medium text-sm text-gray-700'>Notes / Remarks</span>
                <textarea className='border border-gray-300 px-3 py-2 rounded-lg outline-none focus:border-[var(--primary-color)] transition text-sm resize-none' rows={2} value={pricingNotes} onChange={(e) => setPricingNotes(e.target.value)} placeholder='Internal notes for this pricing configuration...' />
              </div>
            </div>
          </div>
          )}
        ...
        <div className='sticky bottom-0 bg-white pt-2 pb-1 flex justify-end gap-3 border-t border-gray-100'>
          <button type='button' onClick={() => setEditBoxOpen(false)} className='border border-gray-300 text-gray-700 px-6 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 transition'>Cancel</button>
